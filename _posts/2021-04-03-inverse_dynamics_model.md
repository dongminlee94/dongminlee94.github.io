...
따라서 이 논문에서의 접근은 simulator와 simulation-based control policy가 주어졌을 때, real robot에 control policy를 바로 적용하는 것이 아닌 <span style="color:red">deep inverse dynamics model</span>을 이용하여 simulator에서 얻은 next state에 가장 적합하고 real system에 알맞은 action을 얻는다. 추가적으로, deep inverse dynamics model를 incremental하게 학습시키기 위해 data collection 방법도 제안한다.
